...
const notMenu: FilterFunc<SimpleSymbol> = (a: SimpleSymbol): boolean =>
  a.name !== 'menu';
...
export function GenBoardDefs(board: BoardsList<SimpleSymbol>): Definition[] {
  const defined: Definition[] = [
    MakeDeclDef('BUILD_PROJECT_NAME', '${PROJ_NAME}', ['PROJ_NAME'], []),
  ];
  board.boards.forEach(
    ({ symbols, menuSelections }: Board<SimpleSymbol>, key: string) => {
