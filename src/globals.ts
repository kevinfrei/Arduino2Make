...
export function MakeGlobals(pt: PlatformTarget): ParsedFile {
  ...
  MakeSymbol(
    ...
    pt.globals.getRuntimePlatformPath,
    scopedTable,
  );
  MakeSymbol(
    'runtime.hardware.path',
    pt.globals.getRuntimeHardwarePath,
    scopedTable,
  );
  MakeSymbol('runtime.ide.path', pt.globals.getRuntimeIdePath, scopedTable);
  ...
  MakeSymbol('runtime.os', pt.globals.getRuntimeOs, scopedTable);
  MakeSymbol('software', 'ARDUINO', scopedTable);
  // {name}: platform vendor name
  MakeSymbol('name', pt.globals.getVendorName, scopedTable);
  // {_id}: board ID of the board being compiled for
  MakeSymbol('_id', pt.globals.getBoardId, scopedTable);
  ...
  MakeSymbol('build.fqbn', pt.globals.getFQBN, scopedTable);
  MakeSymbol('build.source.path', pt.globals.getSourcePath, scopedTable);
  MakeSymbol(
    'build.library_discovery_phase',
    pt.globals.getLibDiscoveryPhase,
    scopedTable,
  );
  MakeSymbol(
    'compiler.optimization_flags',
    pt.globals.getOptFlags,
    scopedTable,
  );
  MakeSymbol(
    'extra.time.utc',
    pt.globals.getTimeUtc(false, false),
    scopedTable,
  );
  // {extra.time.local}: Unix time with local timezone and DST offset
  MakeSymbol(
    'extra.time.local',
    pt.globals.getTimeUtc(true, true),
    scopedTable,
  );
  // {extra.time.zone}: local timezone offset without the DST component
  MakeSymbol(
    'extra.time.zone',
    pt.globals.getTimeUtc(true, false),
    scopedTable,
  );
  // {extra.time.dst}: local daylight savings time offset
  MakeSymbol('extra.time.dst', pt.globals.getTimeUtc(false, true), scopedTable);
